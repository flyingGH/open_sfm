...
bool CheckInitFramePair(const Map &map, FramePair &frame_pair,
    ...
    // Pose pose1 = frame1.Tcw;
    // Pose pose2 = frame2.Tcw;
...
void InitializeMap(Map &map, FramePair &frame_pair) {
    ...
    Eigen::Matrix3d Rc2w = frame2.Tcw.q.toRotationMatrix();
    Eigen::Matrix3d Rwc2 = Rc2w.transpose();
    Eigen::Vector3d t2w = -1.0  * Rwc2 * frame2.Tcw.t;

    Eigen::Matrix3d Rc1w = frame1.Tcw.q.toRotationMatrix();
    Eigen::Matrix3d Rc1c2 = Rc1w * Rwc2;
    Eigen::Vector3d t2c1 = Rc1w * t2w + frame1.Tcw.t;

    frame1.Tcw = Pose(quaternion::Identity(), vector3::Zero());
    frame2.Tcw = Pose(quaternion(Rc1c2), t2c1);
